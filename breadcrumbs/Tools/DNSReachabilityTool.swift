...
struct DNSReachabilityTool: AITool {
    ...
    func execute(arguments: [String: Any]) async throws -> String {
        ...
        Logger.tools("DNSReachabilityTool: domain=\(domain), recordType=\(recordType), dnsServer=\(dnsServer ?? "system"), timeout=\(timeout)")
    ...
    private func checkDNSReachability(
        ...
        dnsServer: String?,
        timeout: Double
    ) async throws -> DNSReachabilityOutput {
        ...
        do {
            ...
            Logger.tools("DNSReachabilityTool.checkDNSReachability: Found \(dnsRecords.count) \(recordType) records for \(domain)")
            
        } catch {
            Logger.tools("DNSReachabilityTool.checkDNSReachability: DNS query failed for \(domain): \(error.localizedDescription)")
        ...
        Logger.tools("DNSReachabilityTool.checkDNSReachability: DNS check completed for \(domain) - reachable: \(isReachable), records: \(dnsRecords.count), time: \(String(format: "%.3f", responseTime))s")
        return result
    }
}
