...
struct AppCheckerTool: AITool {
    ...
    func execute(arguments: [String: Any]) async throws -> String {
        ...
        Logger.tools("AppCheckerTool: parsed input - appName: \(input.appName ?? "nil"), bundleId: \(input.bundleIdentifier ?? "nil"), category: \(input.category ?? "nil")")
    ...
    private func getAllInstalledApps(includeSystemApps: Bool) async throws -> [URL] {
        return await Task.detached {
            ...
            if let userAppsURL = FileManager.default.urls(for: .applicationDirectory, in: .userDomainMask).first {
                let userApps = try await self.getAppsFromDirectory(url: userAppsURL, includeSystemApps: includeSystemApps)
    ...
    private func getAppsFromDirectory(url: URL, includeSystemApps: Bool) async throws -> [URL] {
        return try await Task.detached {
            ...
            let enumerator = fileManager.enumerator(at: url, includingPropertiesForKeys: [.isApplicationKey], options: [.skipsHiddenFiles, .skipsPackageDescendants])
    ...
    private func getAppInfo(from bundleURL: URL?, isRunning: Bool, runningApp: NSRunningApplication?) async -> AppInfo? {
        ...
        let name = bundle.object(forInfoDictionaryKey: kCFBundleNameKey as String) as? String ?? bundleURL.lastPathComponent.replacingOccurrences(of: ".app", with: "")
    ...
    private func getArchitecture(from bundle: Bundle) async -> [String] {
        guard let executablePath = bundle.executablePath else { return [] }
        ...
        return await Task.detached {
            ...
            do {
                ...
                if let output = String(data: data, encoding: .utf8) {
                    ...
                    if output.contains("universal") { architectures.append("universal") }
    ...
    private func getAppCategory(from bundle: Bundle, name: String) -> String? {
        ...
        if lowercasedName.contains("game") || lowercasedName.contains("steam") || lowercasedName.contains("epic") {
            return "games"
        } else if lowercasedName.contains("browser") || lowercasedName.contains("chrome") || lowercasedName.contains("firefox") || lowercasedName.contains("safari") {
            return "web_browsers"
        } else if lowercasedName.contains("security") || lowercasedName.contains("antivirus") || lowercasedName.contains("norton") || lowercasedName.contains("malware") {
            return "security"
        } else if lowercasedName.contains("office") || lowercasedName.contains("word") || lowercasedName.contains("excel") || lowercasedName.contains("powerpoint") {
            return "productivity"
        } else if lowercasedName.contains("media") || lowercasedName.contains("video") || lowercasedName.contains("audio") || lowercasedName.contains("music") {
            return "media"
        } else if lowercasedName.contains("development") || lowercasedName.contains("xcode") || lowercasedName.contains("code") || lowercasedName.contains("programming") {
            return "development"
        }
