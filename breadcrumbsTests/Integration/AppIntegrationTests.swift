...
final class AppIntegrationTests: XCTestCase {
    ...
    @MainActor func testAppModelContainerConfiguration() throws {
        // Given & When
        // Note: breadcrumbsApp is excluded from library target, so we test the core components instead
        let schema = Schema([Item.self])
        let modelContainer = try ModelContainer(
    ...
    @MainActor func testChatViewWithMockModel() throws {
        // Given
        ...
        XCTAssertFalse(try XCTUnwrap(self.persistedViewModel?.isProcessing))
    ...
    func testEndToEndChatFlow() async {
        ...
        XCTAssertEqual(
            ...
            5
        ) // system + user + assistant with tools + tool result + final response
    ...
    func testAppInitializationPerformance() {
        measure {
            _ = try! ModelContainer(for: Item.self, configurations: ModelConfiguration(isStoredInMemoryOnly: true))
        }
    }
