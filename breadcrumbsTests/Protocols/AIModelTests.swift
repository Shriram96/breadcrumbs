...
final class AIModelTests: XCTestCase {
    ...
    func testAIModelSendMessageError() async {
        ...
        // When & Then
        do {
            _ = try await mockModel.sendMessage(messages: messages, tools: nil)
    ...
    @MainActor func testChatMessageCreationPerformance() {
        measure {
            for i in 0..<1000 {
                _ = ChatMessage(role: .user, content: "Message \(i)")
            }
        }
    }
    ...
    @MainActor func testToolCallCreationPerformance() {
        measure {
            for i in 0..<1000 {
                _ = ToolCall(id: "call\(i)", name: "tool\(i)", arguments: "{}")
            }
        }
    }

    @MainActor func testChatMessageCodablePerformance() {
